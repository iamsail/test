...
//特殊字符进行转义
    ...
    var publishNum=0;
    $('.head').click(function(){
         $("#user-headimg").attr('src',$('.head').attr("src")); 
         $('#user-name').html($('.login-state').html());
    ...
    success: function success(data) {
        ...
        if(off3){
             var userInfo = data;
        publishNum=userInfo.length;
        $('#publish-num').html(publishNum);
        $('.publish-data').remove();
        for (var i = 0; i < userInfo.length; i++) {
            $('#user-publish').append("<div class='publish-data'><div class='publish-info1'><span class='publish-headline'></span><p class='publish-content'></p><img class='publish-img1' src='' alt=''><img class='publish-img2' src='' alt=''><span class='publish-start'></span><span class='publish-stop'></span></div><input class='publish-remove' type='button' value='删除'></div>");
            $('.publish-headline').eq(i).html(HtmlEncode(userInfo[i].cate + ':'+userInfo[i].title));
            $('.publish-content').eq(i).html(HtmlEncode(userInfo[i].content));
            $('.publish-start').eq(i).html(HtmlEncode('发布时间:' + userInfo[i].update_time));
            $('.publish-stop').eq(i).html(HtmlEncode('截止时间:' + userInfo[i].overtime));
            $('.publish-remove').eq(i).attr('name', userInfo[i].id);
           if (userInfo[i].path0) {
                $('.publish-img1').eq(i).attr("src", "http://" + config_Address + "/coalball/Application/Common/Common/Uploads/" + userInfo[i].path0 + userInfo[i].savename0);
            } 
            if (userInfo[i].path1) {
                $('.publish-img2').eq(i).attr("src", "http://" + config_Address + "/coalball/Application/Common/Common/Uploads/" + userInfo[i].path1 + userInfo[i].savename1);
            }
            $(".publish-info1:last").on("click",publishInfo);
            $(".publish-remove:last").on("click",removeInfo);
        }
        off3=false;
        }
    ...
    error: function error(jqXHR) {
        ...
        if($('#middle-show').css("display")=='block'){
                off2=2;
        }
        if($('#middle-upload').css("display")=='block'){
                off2=3;
        }
   ...
   function removeInfo(){
    var publishId = $(this).attr('name');
         $.ajax({
    type: "POST",
    url: "Home/Index/deleteUserMessage",
    data:{id:publishId},
    success: function success(data) {
        alert("删除成功!" );
    },
    ...
    // 个人页进入发布信息详情页
function publishInfo(ev){
        $('#middle-bar').append("<div id='detail-page'><span id='page-cate'></span><h4 id='page-headline'></h4><p id='page-body'></p><img id='page-img1' src='' ><img id='page-img2' src=''><span id='timeStart'></span><span id='timeStop'></span><button id='page-btn'>返回我的主页</button></div>");
        $('#middle-detail').css('display','none');
        var pageInfo1 = $(this).find(".publish-headline").text().split(':');
        var pageInfo2 = $(this).find(".publish-content").text();
        var pageInfo3 = $(this).find(".publish-start").text();
        var pageInfo4 = $(this).find(".publish-stop").text();
        var pageInfo5 = $(this).find(".publish-img1").attr('src');
        var pageInfo6 = $(this).find(".publish-img2").attr('src');
        $('#page-cate').html(HtmlEncode(pageInfo1[0]+':'));
        $('#page-headline').html(HtmlEncode(pageInfo1[1]));
        $('#page-body').html(HtmlEncode(pageInfo2));
        $('#timeStart').html(HtmlEncode(pageInfo3));
        $('#timeStop').html(HtmlEncode(pageInfo4));
        $('#page-img1').attr('src',pageInfo5);
